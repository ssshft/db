...
void md::BinanceSbeUnit::subWebsocekt() {
    web::websockets::client::websocket_client_config cfg;
    cfg.headers().add("X-MBX-APIKEY", sbeAccount.apiKey);

START_SUB_SBE_WEBSOCKET(cfg)
...
void md::BinanceSbeUnit::ping(){

}

void md::BinanceSbeUnit::pong() {
    try {
...
void md::BinanceSbeUnit::onWebsocketMsg(const web::websockets::client::websocket_incoming_message& msg) {
    latestDataUpdateTime = crypto::getCurrentTime();

    switch (msg.message_type()) {
        case web::websockets::client::websocket_message_type::binary_message: {
            auto body = msg.body();
            std::vector<uint8_t> buf(msg.length());
            auto task = body.read_to_end(concurrency::streams::container_buffer<std::vector<uint8_t>>(buf));
            task.wait();
        ...
        case web::websockets::client::websocket_message_type::text_message: {
            const string& s = msg.extract_string().get();
            LOG_INFO("Received sbe text msg: {}", msg);
            return;
        }
