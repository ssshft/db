...
void md::BybitUnit::save_md_string(const MDMsg &mdMsg){
    ...
    string originInstId;
    if(rawData["data"].HasMember("s")){//DEPTH1
        originInstId = rawData["data"]["s"].GetString();
    }
    else if(rawData["data"][0].HasMember("s")){//TRADES
    ...
    if(mdMsg.header.instTypeEnum == SPOT){
        ...
        // }
        // else if(mdMsg.header.marketTypeEnum == DEPTH5
        //    || mdMsg.header.marketTypeEnum == DEPTH10
        ...
        if(mdMsg.header.marketTypeEnum == DEPTH1){
            ...
            if(found != _cacheMDMap.end()){
                ...
                string asksStr = fmt::format("[[{:.12f},{:.12f}]]", rcmd.body.depth1.ap1, rcmd.body.depth1.av1);
                string bidsStr = fmt::format("[[{:.12f},{:.12f}]]", rcmd.body.depth1.bp1, rcmd.body.depth1.bv1);
           ...
           || mdMsg.header.marketTypeEnum == DEPTH20 ){
            ...
            for(rapidjson::size_t i = 0; i < data.Size(); i++){
                ...
                cmd.body.trades.px = stod(data[i]["p"].GetString());
                cmd.body.trades.sz = stod(data[i]["v"].GetString());
                ...
                mdStr = fmt::format(Trades_Format_FMT,
                    ...
                    data[i]["i"].GetString(),
                    stod(data[i]["p"].GetString()),
                    stod(data[i]["v"].GetString()),
                    side,
                    ts,
                    mdMsg.tsNet,
                    crypto::getCurrentTime());
            ...
            // const rapidjson::Value &data = rawData["data"];
            // bool isFinished = data["k"]["x"].GetBool();
            // if(!isFinished){
            //     return;
            // }
            // sprintf(mdStr, Bar_Format,
            //         exchIdStr.c_str(), instTypeStr.c_str(), marketTypeStr.c_str(), info.instId,
            //         stol(data["k"]["t"].GetString()) * 1000,
            //         stod(data["k"]["h"].GetString()),
            //         stod(data["k"]["l"].GetString()),
            //         stod(data["k"]["o"].GetString()),
            //         stod(data["k"]["c"].GetString()),
            //         0.0,
            //         stod(data["k"]["v"].GetString()),
            //         stod(data["k"]["q"].GetString()),

            //         stod(data["k"]["V"].GetString()),
            //         stod(data["k"]["Q"].GetString()),
            //         stod(data["k"]["v"].GetString()) - stod(data["k"]["V"].GetString()) ,
            //         stod(data["k"]["q"].GetString()) - stod(data["k"]["Q"].GetString()) ,
            //         stoi(data["k"]["n"].GetString()),
...
void md::BybitMarketClientV2::print_stat() {
    while(1){
        try{
...
void md::BybitMarketClientV2::construct() {
...
void md::BybitMarketClientV2::start(){
    construct();
    for(auto &unit: tokenUnitVec){
        //共用一个smc即可
        unit.smc = smc;
    ...
    std::thread printStatThread(&md::BybitMarketClientV2::print_stat, this);
