...
#define FTX_WEBSOCKET_HOST_PUBLIC_SPOT "wss://ftx.com/ws/"


namespace md {
    struct FTXUnit : public MarketDataBaseStruct{
        FTXUnit(ExchangeType exchangeType, InstType instType, MarketType marketType,
        ...
        void start(){
            ...
            //启动ws监控线程
            std::thread monitorWSThread(&FTXUnit::monitor_ws, this);
            monitorWSThread.detach();
        }
        void subscribe(string &originInstId);
    };

    class FTXMarketClientV2 : public MarketDataBaseClass {
    public:
        FTXMarketClientV2(vector<string> instTypeVec, vector<string> marketTypeVec, vector<string> instIdVec,
